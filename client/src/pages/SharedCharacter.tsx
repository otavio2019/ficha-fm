...
import { observationReviewField } from "@shared/fmObservations";
...
function hydrateSharedSheet(raw: Record<string, unknown> | null | undefined): FMCharacterSheet {
  ...
  return {
    ...
    mechanics: { ...empty.mechanics, ...(source.mechanics ?? {}), race: source.mechanics?.race ? { ...source.mechanics.race, modifiers: Array.isArray(source.mechanics.race.modifiers) ? source.mechanics.race.modifiers : [], requirements: Array.isArray(source.mechanics.race.requirements) ? source.mechanics.race.requirements : [], characteristics: Array.isArray(source.mechanics.race.characteristics) ? source.mechanics.race.characteristics : [], abilities: Array.isArray(source.mechanics.race.abilities) ? source.mechanics.race.abilities : [], choices: Array.isArray(source.mechanics.race.choices) ? source.mechanics.race.choices : [], selectedChoices: Array.isArray(source.mechanics.race.selectedChoices) ? source.mechanics.race.selectedChoices : [], evolutions: Array.isArray(source.mechanics.race.evolutions) ? source.mechanics.race.evolutions.map(evolution => ({ ...evolution, requirements: Array.isArray(evolution.requirements) ? evolution.requirements : [], modifiers: Array.isArray(evolution.modifiers) ? evolution.modifiers : [], characteristics: Array.isArray(evolution.characteristics) ? evolution.characteristics : [], abilities: Array.isArray(evolution.abilities) ? evolution.abilities : [], choices: Array.isArray(evolution.choices) ? evolution.choices : [] })) : [] } : null },
...
export default function SharedCharacter() {
  ...
  const observationOptions = [
    sheet.technique.notes ? { label: `Técnica · ${sheet.technique.name || "sem nome"}`, field: observationReviewField("technique", "technique"), currentValue: sheet.technique.notes } : null,
    ...(sheet.domainExpansion?.notes ? [{ label: `Domínio · ${sheet.domainExpansion.name || "sem nome"}`, field: observationReviewField("domain", "domain-expansion"), currentValue: sheet.domainExpansion.notes }] : []),
    ...sheet.aptitudes.filter(item => Boolean(item.notes)).map(item => ({ label: `Aptidão · ${item.name}`, field: observationReviewField("aptitude", item.id), currentValue: item.notes ?? "" })),
    ...sheet.training.filter(item => Boolean(item.notes)).map(item => ({ label: `Treinamento · ${item.label ?? item.trackId}`, field: observationReviewField("training", item.trackId), currentValue: item.notes })),
    ...sheet.invocations.filter(item => Boolean(item.notes)).map(item => ({ label: `Invocação · ${item.name}`, field: observationReviewField("invocation", item.id), currentValue: item.notes })),
    ...sheet.equipment.filter(item => Boolean(item.notes)).map(item => ({ label: `Equipamento · ${item.name}`, field: observationReviewField("equipment", item.id), currentValue: item.notes })),
    ...sheet.cursedTools.filter(item => Boolean(item.notes)).map(item => ({ label: `Ferramenta · ${item.name}`, field: observationReviewField("equipment", item.id), currentValue: item.notes })),
    ...sheet.allies.filter(item => Boolean(item.notes)).map(item => ({ label: `Aliado · ${item.name}`, field: observationReviewField("ally", item.id), currentValue: item.notes })),
  ...
  return <main className="min-h-screen bg-[#09060f] px-4 py-7 text-stone-100 sm:px-6 sm:py-10"><div className="mx-auto max-w-6xl"><header className="mb-7 rounded-2xl border border-amber-300/15 bg-[radial-gradient(circle_at_88%_10%,rgba(151,93,199,.18),transparent_36%),#120c1d] p-6 sm:p-8"><div className="flex flex-wrap items-center gap-2 text-xs uppercase tracking-[.2em] text-amber-300/70"><MoonStar className="h-4 w-4" />Arquivo compartilhado · somente leitura</div><div className="mt-5 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="font-display text-4xl text-stone-100 sm:text-5xl">{sheet.identity.name || sharedQuery.data.name}</h1><p className="mt-2 text-sm text-stone-400">{sheet.identity.player || "Jogador não informado"}{sheet.identity.grade ? ` · ${sheet.identity.grade}` : ""}{sheet.origin.name ? ` · ${sheet.origin.name}` : ""}</p></div><p className="text-xs leading-5 text-stone-500">Esta página se atualiza quando o proprietário salva alterações.</p></div></header>
    ...
    {observationOptions.length ? <ReadCard title="Observações compartilhadas" icon={<BookOpen className="h-4 w-4" />}><div className="space-y-3">{observationOptions.map(item => <div key={item.field} className="rounded-xl border border-violet-300/10 bg-black/20 p-3"><p className="text-xs uppercase tracking-[.14em] text-amber-200">{item.label}</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-stone-300">{item.currentValue}</p></div>)}</div></ReadCard> : null}
    ...
    <div className="mt-4"><PublicReviewForm token={token} targetName={sheet.identity.name || sharedQuery.data.name} observationOptions={observationOptions} /></div>
  </div></main>;
}
