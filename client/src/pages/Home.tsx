...
import { FM_ATTRIBUTE_LABELS, FM_SAVING_THROW_LABELS, FM_SPECIALIZATION_LABELS, getAttackBonus, getDerivedValues, getHighestSpellLevel, getResourceLabel, getSkillBonus, getSpellCost, getSustainCost, getTechniquePowerProgression, rollD20 } from "@shared/fmRules";
...
import { applyInfiniteWorldMission, getExperienceForLevel, getInfiniteWorldProgress, getMissionExperienceReward, getMissionInterludeReward, getMissionMoneyReward, type InfiniteWorldMissionDifficulty, type InfiniteWorldMoneyDifficulty } from "@shared/infiniteWorlds";
...
type TabId = "overview" | "attributes" | "skills" | "technique" | "spells" | "invocations" | "combat" | "equipment" | "progression" | "missions" | "house" | "diary";
...
const navigationGroups: Array<{ label: string; items: SheetNavItem[] }> = [
  ...
  { label: "Capacidades", items: [{ id: "skills", label: "Perícias e treinamento", icon: ScrollText }, { id: "technique", label: "Técnica e vínculo", icon: WandSparkles }, { id: "spells", label: "Poderes e feitiços", icon: WandSparkles }, { id: "invocations", label: "Invocações", icon: WandSparkles }] },
  { label: "Aventura", items: [{ id: "combat", label: "Combate e ataques", icon: Swords }, { id: "equipment", label: "Equipamento e carga", icon: Shield }] },
  { label: "Campanha", items: [{ id: "progression", label: "Progressão e grau", icon: ScrollText }, { id: "missions", label: "Missões e Interlúdios", icon: Swords }, { id: "house", label: "Regras da Casa", icon: Shield }, { id: "diary", label: "Diário e registros", icon: BookOpen }] },
];
...
export default function Home() {
  ...
  const [tab, setTab] = useState<TabId>(() => {
    ...
    return ["overview", "attributes", "skills", "technique", "spells", "invocations", "combat", "equipment", "progression", "missions", "house", "diary"].includes(requested ?? "") ? requested! : "overview";
  });
  ...
  const [previewTechniqueSheet, setPreviewTechniqueSheet] = useState<FMCharacterSheet>(() => {
    ...
    previewSheet.progression = { ...previewSheet.progression, level: 3, specializationLevels: 3, specialization: "technique-specialist" };
  ...
  useEffect(() => {
    if (previewMode && !previewLibraryMode) {
      ...
      initialSheet.progression = { ...initialSheet.progression, level: 3, specializationLevels: 3, specialization: "technique-specialist" };
...
function CharacterLibrary({ userName, characters, sharedCount, loading, creating, newName, onNewName, onCreate, onOpen, onDuplicate, onDelete, techniqueCharacterId, techniqueTarget, techniqueLoading, onTechniqueCharacterChange, onSaveTechnique, techniques, techniquesLoading, onSaveIndependentTechnique, onRemoveIndependentTechnique, onToggleCreate, onLogout }: {
  ...
  const [libraryTab, setLibraryTab] = useState<"characters" | "techniques">(() => new URLSearchParams(window.location.search).get("preview") === "library-techniques" ? "techniques" : "characters");
...
function GurpsDossierFrame({ tab, children }: { tab: TabId; children: React.ReactNode }) {
  ...
  const flow: Record<TabId, string> = { overview: "Identidade → recursos → referências", attributes: "Características → modificadores → resistências", skills: "Perícia → atributo → treinamento", technique: "Vínculo → limites → poderes derivados", spells: "Poder → custo → resolução", invocations: "Grau → atributos → ações", combat: "Ataque → defesa → cena", equipment: "Item → carga → observações", progression: "XP → nível → grau", missions: "Dificuldade → recompensa → Interlúdios", house: "Regra → registro → consequência", diary: "Rolagem → evento → memória" };
...
function GurpsSectionLedger({ tab, sheet, derived }: { tab: TabId; sheet: FMCharacterSheet; derived: ReturnType<typeof getDerivedValues> }) {
  ...
  const entries: Record<TabId, Array<[string, string]>> = {
    ...
    missions: [["Interlúdios", String(sheet.houseRules.downtime.interludes)], ["Exaustão", String(sheet.houseRules.rest.exhaustion)], ["Missões", String(sheet.houseRules.rest.missionCount)]],
...
function renderTab({ tab, sheet, derived, updateSheet, addDiary, newNote, setNewNote, characterId, previewMode, uploadImage, techniques }: { tab: TabId; sheet: FMCharacterSheet; derived: ReturnType<typeof getDerivedValues>; updateSheet: (updater: (current: FMCharacterSheet) => FMCharacterSheet) => void; addDiary: (title: string, detail: string, category?: FMCharacterSheet["diary"][number]["category"]) => void; newNote: string; setNewNote: (value: string) => void; characterId: string; previewMode: boolean; uploadImage: (input: { characterId: string; fileName: string; contentType: "image/jpeg" | "image/png" | "image/webp"; base64: string; caption: string }) => Promise<FMImageAttachment>; techniques: Array<{ id: string; name: string; technique: Record<string, unknown> }>; }) {
  const content = tab === "overview" ? <OverviewTab sheet={sheet} derived={derived} updateSheet={updateSheet} addDiary={addDiary} characterId={characterId} previewMode={previewMode} uploadImage={uploadImage} techniques={techniques} />
    ...
    : tab === "progression" ? <ProgressionTab sheet={sheet} updateSheet={updateSheet} />
...
export function ImageAttachmentsPanel({ sheet, updateSheet, addDiary, characterId, previewMode, uploadImage }: { sheet: FMCharacterSheet; updateSheet: (updater: (current: FMCharacterSheet) => FMCharacterSheet) => void; addDiary: (title: string, detail: string, category?: FMCharacterSheet["diary"][number]["category"]) => void; characterId: string; previewMode: boolean; uploadImage: (input: { characterId: string; fileName: string; contentType: "image/jpeg" | "image/png" | "image/webp"; base64: string; caption: string }) => Promise<FMImageAttachment> }) {
  ...
  const upload = async (file: File | undefined) => {
    ...
    try {
      ...
      updateSheet(current => ({ ...current, images: [...current.images, image], identity: { ...current.identity, portraitUrl: current.identity.portraitUrl ?? image.url } }));
  ...
  const removeImage = (image: FMImageAttachment) => updateSheet(current => ({ ...current, images: current.images.filter(item => item.id !== image.id), identity: { ...current.identity, portraitUrl: current.identity.portraitUrl === image.url ? null : current.identity.portraitUrl }, diary: [{ id: id(), at: Date.now(), category: "note", title: "Imagem removida", detail: `${image.name} foi removida da galeria.`, }, ...current.diary] }));
  const setPortrait = (image: FMImageAttachment) => updateSheet(current => ({ ...current, identity: { ...current.identity, portraitUrl: image.url } }));
  return <Panel className="mt-4 border-amber-300/15 bg-amber-300/[.025]"><SectionTitle eyebrow="Referências visuais" title="Galeria da ficha" description="Anexe referências do personagem, armas, símbolos ou cenas. JPEG, PNG ou WebP de até 5 MB; os arquivos ficam vinculados somente à sua ficha." action={<label className={`inline-flex h-9 cursor-pointer items-center justify-center rounded-lg border border-violet-300/15 bg-[#20122e] px-3 text-sm text-violet-100 transition hover:border-amber-300/45 hover:text-amber-100 ${uploading || previewMode ? "pointer-events-none opacity-60" : ""}`}><ImagePlus className="mr-2 h-4 w-4" />{uploading ? "Enviando…" : "Anexar imagem"}<input className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" onChange={event => { void upload(event.target.files?.[0]); event.currentTarget.value = ""; }} disabled={uploading || previewMode} /></label>} />{sheet.images.length === 0 ? <div className="rounded-xl border border-dashed border-violet-300/15 bg-black/20 p-5 text-sm leading-6 text-stone-500">Nenhuma imagem anexada. Use a galeria para reunir referências visuais da campanha.</div> : <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{sheet.images.map(image => <div key={image.id} className="overflow-hidden rounded-xl border border-violet-300/15 bg-black/20"><img src={image.url} alt={image.caption || image.name} className="h-48 w-full object-cover" /><div className="space-y-3 p-3"><Input value={image.caption} onChange={event => updateSheet(current => ({ ...current, images: current.images.map(item => item.id === image.id ? { ...item, caption: event.target.value } : item) }))} placeholder="Legenda ou uso na cena" aria-label={`Legenda de ${image.name}`} /><div className="flex flex-wrap gap-2"><ActionButton title="Usar como retrato" onClick={() => setPortrait(image)} className={sheet.identity.portraitUrl === image.url ? "border-amber-300/45 text-amber-100" : ""}>{sheet.identity.portraitUrl === image.url ? "Retrato atual" : "Usar como retrato"}</ActionButton><ActionButton title="Remover imagem" onClick={() => removeImage(image)} className="hover:border-red-400/60 hover:text-red-200"><Trash2 className="h-4 w-4" /></ActionButton></div></div></div>)}</div>}</Panel>;
}
...
function ProgressionTab({ sheet, updateSheet }: { sheet: FMCharacterSheet; updateSheet: (updater: (current: FMCharacterSheet) => FMCharacterSheet) => void }) {
  ...
  return <><SectionTitle eyebrow="Campanha · progresso pessoal" title="Progressão de Grau e Nível" description="XP define automaticamente nível e grau conforme a tabela Infinite Worlds. Missões e seus registros ficam na categoria própria de Campanha." /><Panel className="border-amber-300/20 bg-amber-300/[.035]"><div className="flex flex-col gap-3 border-b border-amber-300/10 pb-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="font-display text-xs uppercase tracking-[.2em] text-amber-300/70">Guilda Infinite Worlds</p><h3 className="mt-1 font-display text-2xl text-stone-100">Trilha de progressão</h3></div><span className="w-fit rounded-full border border-amber-300/25 bg-amber-300/10 px-3 py-1 text-sm font-medium text-amber-100">{progress.grade.label}</span></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><FormulaCard label="XP atual" value={progress.experience} formula="Acumulado na guilda" source="Tabela Infinite Worlds" /><FormulaCard label="Nível" value={progress.level} formula={`Faixa ${progress.grade.minLevel}–${progress.grade.maxLevel}`} source="Tabela Infinite Worlds" /><FormulaCard label="Próximo nível" value={progress.nextLevelExperience ?? "Máximo"} formula={progress.experienceToNextLevel === null ? "Nível 30 consolidado" : `Faltam ${progress.experienceToNextLevel} XP`} source="Tabela Infinite Worlds" /><FormulaCard label="Moeda" value={formatCurrency(sheet.guild?.currency ?? 0)} formula="Recompensas acumuladas" source="Tabela Infinite Worlds" /><FormulaCard label="Faixa de XP" value={`${progress.grade.minExperience}–${progress.grade.maxExperience}`} formula="Faixa oficial do grau" source="Tabela Infinite Worlds" /></div><Panel className="mt-4 border-violet-300/10 bg-black/20"><Field label="XP acumulado" hint="Ajustes manuais atualizam nível e grau; para recompensas use Missões e Interlúdios."><Input type="number" min={0} max={6499} value={progress.experience} onChange={event => applyExperience(event.target.value)} /></Field></Panel></Panel></>;
}
...
function MissionsTab({ sheet, updateSheet, addDiary }: { sheet: FMCharacterSheet; updateSheet: (updater: (current: FMCharacterSheet) => FMCharacterSheet) => void; addDiary: (title: string, detail: string, category?: FMCharacterSheet["diary"][number]["category"]) => void }) {
  ...
  const applyMissionReward = () => {
    const resolution = applyInfiniteWorldMission(sheet, xpDifficulty, moneyDifficulty);
    const { experience, money, interludes } = resolution.rewards;
    updateSheet(current => {
      return applyInfiniteWorldMission(current, xpDifficulty, moneyDifficulty).sheet;
    });
    addDiary(`Missão concluída — ${resolution.rewards.grade}`, `+${experience} XP (${xpDifficulty}), ${formatCurrency(money)} (${moneyDifficulty}${sheet.houseRules.dedicationRewarding ? "; Dedicação Recompensadora" : ""}) e +${interludes} Interlúdio(s). Exaustão e contagem de missão foram atualizadas.`, "note");
    toast.success(`Recompensa registrada: +${experience} XP, ${formatCurrency(money)} e +${interludes} Interlúdio(s).`);
  };
  return <><SectionTitle eyebrow="Campanha · resolução de missão" title="Missões e Interlúdios" description="Registre a missão por categoria. Difícil concede 1 Interlúdio; Difícil+ concede 1,5. As demais dificuldades não concedem Interlúdios." /><Panel className="border-amber-300/20 bg-amber-300/[.035]"><p className="font-display text-xs uppercase tracking-[.18em] text-amber-300/70">Recompensas disponíveis</p><div className="mt-4 grid gap-3 sm:grid-cols-3"><FormulaCard label="XP" value={`+${getMissionExperienceReward(progress.grade.id, xpDifficulty)}`} formula={`${progress.grade.label} · ${xpDifficulty}`} source="Tabela Infinite Worlds" /><FormulaCard label="Moeda" value={formatCurrency(getMissionMoneyReward(progress.grade.id, moneyDifficulty, sheet.houseRules.dedicationRewarding))} formula={sheet.houseRules.dedicationRewarding ? "Dedicação Recompensadora" : `${progress.grade.label} · ${moneyDifficulty}`} source="Tabela Infinite Worlds" /><FormulaCard label="Interlúdios" value={`+${getMissionInterludeReward(xpDifficulty)}`} formula={xpDifficulty === "hard" ? "Difícil" : xpDifficulty === "hard-plus" ? "Difícil+" : "Sem recompensa nesta dificuldade"} source="Regra da guilda" /></div></Panel><Panel className="mt-4"><div className="grid gap-3 md:grid-cols-3"><Field label="Dificuldade da missão"><select className="h-10 rounded-md border border-input bg-background px-3 text-sm text-foreground" value={xpDifficulty} onChange={event => setXpDifficulty(event.target.value as InfiniteWorldMissionDifficulty)}><option value="easy">Fácil</option><option value="medium">Médio</option><option value="hard">Difícil · +1 Interlúdio</option><option value="hard-plus">Difícil+ · +1,5 Interlúdios</option></select></Field><Field label="Categoria de moeda"><select className="h-10 rounded-md border border-input bg-background px-3 text-sm text-foreground" value={moneyDifficulty} onChange={event => setMoneyDifficulty(event.target.value as InfiniteWorldMoneyDifficulty)}><option value="easy">Fácil</option><option value="normal">Normal</option><option value="hard">Difícil</option></select></Field><div className="flex items-end"><Button type="button" onClick={applyMissionReward} className="w-full bg-amber-300 text-[#190d07] hover:bg-amber-200">Registrar missão</Button></div></div><p className="mt-4 rounded-xl border border-violet-300/10 bg-black/20 p-3 text-sm leading-6 text-stone-400">Ao registrar, a ficha recebe XP, moeda, Interlúdios aplicáveis, +1 Exaustão e +1 missão no descanso. O lançamento é incluído no Diário.</p></Panel></>;
}
...
function SpellsTab({ sheet, derived, updateSheet, addDiary }: { sheet: FMCharacterSheet; derived: ReturnType<typeof getDerivedValues>; updateSheet: (updater: (current: FMCharacterSheet) => FMCharacterSheet) => void; addDiary: (title: string, detail: string, category?: FMCharacterSheet["diary"][number]["category"]) => void }) {
  ...
  const addTechniquePower = (power: FMTechniquePower) => {
    ...
    updateSheet(current => ({ ...current, spells: [...current.spells, { id: id(), sourcePowerId: power.id, name: power.name, type: power.type, level: power.spellLevel, casting: "common", reach: "12 metros", targetOrArea: "Uma criatura", durationType: "immediate", durationDetail: "", effect: power.summary, counterplay: "", requirement: power.requirement, damage: "", damageType: "", resolution: "attack", savingThrow: "", costAdjustment: 0, combatModifierTarget: "none", combatModifier: 0, notes: "", active: false }] }));
