...
import { CharacterObservationsPanel } from "@/components/CharacterObservationsPanel";
...
function OverviewTab({ sheet, derived, updateSheet, addDiary, characterId, previewMode, uploadImage, techniques }: { sheet: FMCharacterSheet; derived: ReturnType<typeof getDerivedValues>; updateSheet: (updater: (current: FMCharacterSheet) => FMCharacterSheet) => void; addDiary: (title: string, detail: string, category?: FMCharacterSheet["diary"][number]["category"]) => void; characterId: string; previewMode: boolean; uploadImage: (input: { characterId: string; fileName: string; contentType: "image/jpeg" | "image/png" | "image/webp"; base64: string; caption: string }) => Promise<FMImageAttachment>; techniques: Array<{ id: string; name: string; technique: Record<string, unknown> }> }) {
  ...
  return <><SectionTitle eyebrow="Núcleo do personagem" title="Visão geral" description="Acompanhe identidade, retrato principal, recursos atuais e as fórmulas que sustentam a cena." />
    ...
    <CharacterObservationsPanel sheet={sheet} onUpdate={updateSheet} onDiary={addDiary} />
    ...
    <div className="mt-4"><Panel><SectionTitle eyebrow="História de personagem" title="Os acontecimentos que definem a alma" description="Registre origem, acontecimentos importantes, personalidade, objetivos e passado em um espaço próprio." /><Textarea className="min-h-72" value={sheet.characterHistory ?? ""} onChange={event => updateSheet(current => ({ ...current, characterHistory: event.target.value }))} placeholder="Conte a história do personagem…" /></Panel></div>
...
function SpecializationAbilitiesPanel({ sheet, updateSheet }: { sheet: FMCharacterSheet; updateSheet: (updater: (current: FMCharacterSheet) => FMCharacterSheet) => void }) {
  ...
  const unlockedCount = progress.reduce((total, entry) => total + entry.automatic.length + entry.choiceSlots.filter(slot => Boolean(slot.selectedAbility)).length, 0);
  const pendingCount = progress.reduce((total, entry) => total + entry.choiceSlots.filter(slot => !slot.selectedAbility).length, 0);
  const upcomingCount = progress.reduce((total, entry) => total + entry.upcomingChoiceSlots.length, 0);
  return <Panel className="mt-4"><SectionTitle eyebrow="Progressão rastreável" title="Habilidades da Especialização" description="Marcos automáticos são liberados pelo núcleo e nível; escolhas pendentes não aplicam efeitos até a confirmação. O catálogo permanece rastreável e sem benefícios inventados." />
    <div className="mb-4 grid gap-2 sm:grid-cols-3"><div className="rounded-lg border border-emerald-300/15 bg-emerald-300/[.04] px-3 py-2"><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-emerald-200/70">Desbloqueadas</p><p className="mt-1 font-display text-lg text-emerald-100">{unlockedCount}</p></div><div className="rounded-lg border border-amber-300/15 bg-amber-300/[.04] px-3 py-2"><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-amber-200/70">Escolhas pendentes</p><p className="mt-1 font-display text-lg text-amber-100">{pendingCount}</p></div><div className="rounded-lg border border-violet-300/15 bg-violet-300/[.04] px-3 py-2"><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-violet-200/70">Próximas escolhas</p><p className="mt-1 font-display text-lg text-violet-100">{upcomingCount}</p></div></div>
...
function SpellsTab({ sheet: characterSheet, derived, updateSheet: updateCharacterSheet, addDiary }: { sheet: FMCharacterSheet; derived: ReturnType<typeof getDerivedValues>; updateSheet: (updater: (current: FMCharacterSheet) => FMCharacterSheet) => void; addDiary: (title: string, detail: string, category?: FMCharacterSheet["diary"][number]["category"]) => void }) {
  ...
  const updateSheet = (updater: (current: FMCharacterSheet) => FMCharacterSheet) => updateCharacterSheet(current => {
    ...
    const next = updater(getEffectiveMutantSheet(current));
    return { ...current, diary: next.diary, mutantCores: { ...current.mutantCores, cores: current.mutantCores.cores.map(core => core.id === active.id ? { ...core, attributes: next.attributes.base, resources: next.resources, spells: next.spells, specialization: next.progression.specialization, specializationAbilityChoices: next.progression.specializationAbilityChoices ?? core.specializationAbilityChoices, specializationAbilityUnlocks: next.progression.specializationAbilityUnlocks ?? core.specializationAbilityUnlocks } : core) } };
  });
  const effectiveSheet = getEffectiveMutantSheet(sheet);
  const activeCore = isMutantCursedCorpse(sheet) ? getMutantActiveCore(sheet.mutantCores) : null;
