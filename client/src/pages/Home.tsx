...
export default function Home() {
  ...
  const activeQuery = trpc.characters.get.useQuery({ id: activeCharacterId ?? "sem-ficha" }, { enabled: isAuthenticated && Boolean(activeCharacterId) });
  ...
  }, [activeCharacterId, activeQuery.data]);
  ...
  }, [activeCharacterId, refetchActiveCharacter, refetchCharacterLibrary]);

  useEffect(() => {
    if (!sheet || !activeCharacterId || !isAuthenticated || sheet.skills.some(skill => !skill.name.trim())) return;
  ...
  }, [activeCharacterId, isAuthenticated, sheet]);
...
function OverviewTab({ sheet, derived, updateSheet }: { sheet: FMCharacterSheet; derived: ReturnType<typeof getDerivedValues>; updateSheet: (updater: (current: FMCharacterSheet) => FMCharacterSheet) => void }) {
  ...
  return <><SectionTitle eyebrow="Núcleo do personagem" title="Visão geral" description="Acompanhe a identidade, os recursos atuais e as fórmulas que sustentam a cena." />
    ...
    <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><FormulaCard label="Defesa" value={derived.defense} formula="10 + Destreza + metade do nível + bônus" /><FormulaCard label="Iniciativa" value={`${derived.initiative >= 0 ? "+" : ""}${derived.initiative}`} formula="Destreza + bônus" /><FormulaCard label="Atenção" value={derived.attention} formula="10 + Percepção + bônus" /><FormulaCard label="CD da técnica" value={derived.techniqueDc} formula="10 + metade do nível + atributo + treinamento" /></div>
...
function FormulaCard({ label, value, formula }: { label: string; value: string | number; formula: string }) { return <Panel className="border-violet-300/10"><p className="text-xs uppercase tracking-[0.16em] text-stone-500">{label}</p><p className="mt-1 font-display text-3xl text-amber-200">{value}</p><p className="mt-2 text-xs leading-5 text-stone-500">{formula}</p></Panel>; }
...
function AttributesTab({ sheet, derived, updateSheet }: { sheet: FMCharacterSheet; derived: ReturnType<typeof getDerivedValues>; updateSheet: (updater: (current: FMCharacterSheet) => FMCharacterSheet) => void }) {
  return <><SectionTitle eyebrow="Estrutura de poder" title="Atributos e progressão" description="Atributos de 0 a 30; o limite natural é 20. As fórmulas mostram o efeito do valor atual." />
    ...
    <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><FormulaCard label="PV máximo" value={derived.healthMaximum} formula="Especialização + Constituição por nível + bônus" /><FormulaCard label={getResourceLabel(sheet.progression.specialization, sheet.progression.nonSorcerer)} value={derived.energyMaximum} formula="Especialização por nível, ou Estamina limitada quando não-feiticeiro" /><FormulaCard label="Integridade da Alma" value={derived.integrity} formula="Igual ao máximo de PV" /><FormulaCard label="Deslocamento" value={`${derived.movement} m`} formula="9 m + bônus registrados" /></div>
    <Panel className="mt-4"><div className="mb-4"><p className="font-display text-xs uppercase tracking-[.18em] text-amber-300/70">Testes de Resistência</p><h3 className="mt-1 font-display text-xl text-stone-100">Resistências/TRs</h3><p className="mt-1 text-sm text-stone-500">Atributo-chave + metade do nível + treinamento quando aplicável.</p></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{fmSavingThrowKeys.map(key => <label key={key} className="flex items-center justify-between gap-3 rounded-xl border border-violet-300/10 bg-black/20 p-3"><span><span className="block text-sm font-medium text-stone-200">{FM_SAVING_THROW_LABELS[key]}</span><span className="mt-1 block font-display text-xl text-amber-200">{derived.savingThrows[key] >= 0 ? "+" : ""}{derived.savingThrows[key]}</span></span><input type="checkbox" className="h-4 w-4 accent-amber-300" checked={sheet.progression.savingThrowTraining[key]} onChange={event => updateSheet(current => ({ ...current, progression: { ...current.progression, savingThrowTraining: { ...current.progression.savingThrowTraining, [key]: event.target.checked } } }))} aria-label={`Treinado em ${FM_SAVING_THROW_LABELS[key]}`} /> </label>)}</div></Panel></>;
}
...
function SkillsTab({ sheet, derived, updateSheet }: { sheet: FMCharacterSheet; derived: ReturnType<typeof getDerivedValues>; updateSheet: (updater: (current: FMCharacterSheet) => FMCharacterSheet) => void }) {
  ...
  return <><SectionTitle eyebrow="Testes de perícia" title="Perícias" description={hasInvalidSkills ? "Cada perícia precisa de um nome antes de a ficha poder ser salva." : "Bônus = atributo-chave + metade do nível + treinamento (ou mestria) + outros bônus."} action={<Button size="sm" onClick={addSkill} className="bg-amber-300 text-[#190d07] hover:bg-amber-200"><Plus className="mr-2 h-4 w-4" />Adicionar perícia</Button>} />
...
function SpellsTab({ sheet, derived, updateSheet, addDiary }: { sheet: FMCharacterSheet; derived: ReturnType<typeof getDerivedValues>; updateSheet: (updater: (current: FMCharacterSheet) => FMCharacterSheet) => void; addDiary: (title: string, detail: string, category?: FMCharacterSheet["diary"][number]["category"]) => void }) {
  ...
  return <><SectionTitle eyebrow="Perfil amaldiçoado" title="Magias/Maldições" description="Em F&M, o termo oficial interno é Feitiço. Custos, requisitos e efeitos especiais ficam explícitos para validação do Narrador." action={<Button size="sm" onClick={addSpell} className="bg-amber-300 text-[#190d07] hover:bg-amber-200"><Plus className="mr-2 h-4 w-4" />Adicionar feitiço</Button>} />
    ...
    <div className="mt-4 grid gap-4 md:grid-cols-3"><FormulaCard label="CD da técnica" value={derived.techniqueDc} formula="Usada por feitiços com teste de resistência" /><FormulaCard label="Atributo da técnica" value={FM_ATTRIBUTE_LABELS[sheet.progression.techniqueAttribute]} formula="Definido no funcionamento básico" /><FormulaCard label="Energia atual" value={`${sheet.resources.energy.current}/${derived.energyMaximum}`} formula="O uso de feitiço consome PE conforme o custo" /></div></>;
}
...
function DiaryTab({ sheet, derived, updateSheet, newNote, setNewNote, addDiary }: { sheet: FMCharacterSheet; derived: ReturnType<typeof getDerivedValues>; updateSheet: (updater: (current: FMCharacterSheet) => FMCharacterSheet) => void; newNote: string; setNewNote: (value: string) => void; addDiary: (title: string, detail: string, category?: FMCharacterSheet["diary"][number]["category"]) => void }) {
  ...
  return <><SectionTitle eyebrow="Memória da sessão" title="Diário" description="Cada ajuste de recurso e rolagem é preservado. F&M usa rolagens de d20; vantagem e desvantagem escolhem, respectivamente, o maior e o menor resultado." />
