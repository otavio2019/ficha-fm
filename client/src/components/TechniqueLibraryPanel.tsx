...
export function TechniqueLibraryPanel({ techniques, loading, onSave, onRemove }: Props) {
  ...
  const addPower = () => update(current => ({ ...current, powers: [...current.powers, { id: crypto.randomUUID(), name: "Novo poder", requiredCharacterLevel: 1, spellLevel: 1, type: "damage", summary: "", requirement: "" }] }));
  const save = async () => {
    if (!ready) return;
    setSaving(true);
    try {
