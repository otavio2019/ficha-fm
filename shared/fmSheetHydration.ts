...
export function hydrateSheet(raw: Record<string, unknown> | null | undefined): FMCharacterSheet {
  ...
  return {
    ...
    invocations: Array.isArray(source.invocations) ? source.invocations.map(invocation => ({ ...invocation, type: invocation?.type === "puppet" || invocation?.type === "tamed-curse" || invocation?.type === "shikigami" ? invocation.type : "shikigami" })) : [],
    ...
    deathSaves: { successes: Math.max(0, Number(source.deathSaves?.successes) || 0), failures: Math.max(0, Number(source.deathSaves?.failures) || 0), stabilized: source.deathSaves?.stabilized === true, notes: typeof source.deathSaves?.notes === "string" ? source.deathSaves.notes : "" },
    damageReductions: Array.isArray(source.damageReductions) ? source.damageReductions : [],
    resistances: Array.isArray(source.resistances) ? source.resistances.filter(value => typeof value === "string") : [],
    vulnerabilities: Array.isArray(source.vulnerabilities) ? source.vulnerabilities.filter(value => typeof value === "string") : [],
    inspiration: typeof source.inspiration === "number" ? Math.max(0, source.inspiration) : empty.inspiration,
    energyLimit: typeof source.energyLimit === "number" ? Math.max(0, source.energyLimit) : empty.energyLimit,
